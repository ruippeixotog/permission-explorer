package pt.up.fe.ssin.pexplorer.contentprovider;
...
public class PermissionExplorerContentProvider extends ContentProvider {
    ...
    public static final String AUTHORITY = "pt.up.fe.ssin.pexplorer.contentprovider.permissionexplorercontentprovider";
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            db.execSQL("DROP TABLE IF EXISTS " + GlobalSchema.PERMISSION_TABLE_NAME);
            onCreate(db);
        }
    }
    ...
    public int delete(Uri uri, String where, String[] whereArgs) {
        ...
        switch (sUriMatcher.match(uri)) {
            case PERMISSION:
                count = db.delete(GlobalSchema.PERMISSION_TABLE_NAME, where, whereArgs);
                break;
            case PERM_ID:
            	String id = uri.getLastPathSegment();
                count = db.delete(GlobalSchema.PERMISSION_TABLE_NAME,  PermissionExplorerContentProvider.PERMISSION_ID + "= ?", new String[]{id});
                break;
            default:
    ...
    public Uri insert(Uri uri, ContentValues initialValues) {
        if (sUriMatcher.match(uri) != PERMISSION) { throw new IllegalArgumentException("Unknown URI " + uri); }
        ...
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        long rowId = db.insert(GlobalSchema.PERMISSION_TABLE_NAME, null, values);
        if (rowId > 0) {
            Uri noteUri = ContentUris.withAppendedId(PermissionExplorerContentProvider.CONTENT_URI, rowId);
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        ...
        switch (sUriMatcher.match(uri)) {
            ...
            case PERM_ID:
            	String id = uri.getLastPathSegment();
                c = qb.query(db, projection, PermissionExplorerContentProvider.PERMISSION_ID + "= ?", new String[]{id}, null, null, null);
                break;
            default:
                throw new IllegalArgumentException("Unknown URI " + uri);
        }
    ...
    public int update(Uri uri, ContentValues values, String where, String[] whereArgs) {
        ...
        switch (sUriMatcher.match(uri)) {
            case PERMISSION:
                count = db.update(GlobalSchema.PERMISSION_TABLE_NAME, values, where, whereArgs);
                break;
            case PERM_ID:
            	String id = uri.getLastPathSegment();
                count = db.update(GlobalSchema.PERMISSION_TABLE_NAME, values,  PermissionExplorerContentProvider.PERMISSION_ID + "= ?", new String[]{id});
                break;
            default:
                throw new IllegalArgumentException("Unknown URI " + uri);
        }
    ...
    static {
        ...
        permissionProjectionMap.put(PERMISSION_DESCRIPTION, PERMISSION_DESCRIPTION);
